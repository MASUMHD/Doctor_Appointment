...
const AllUsers = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <img
          src={row.original.profilePicture || 'https://cdn-icons-png.flaticon.com/512/2919/2919906.png'} 
          alt="Profile"
  ...
  const handleRoleChange = (user) => {
    Swal.fire({
      ...
      try {
        ...
        const response = await useAxiosPublic.put(`/users/${user._id}`, { role: newRole });
        ...
        if (response.status === 200) {
          ...
          Swal.fire("Updated!", `${user.name}'s role has been changed to ${newRole}.`, "success");
        }
      } catch (error) {
